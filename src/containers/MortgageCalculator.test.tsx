...
test('MortgageCalculator Monthly Down Payment Amount Field', () => {
  render(<MortgageCalculator />);
  const amount = screen.getByTestId('mortgageMonthlyDownPaymentAmount') as HTMLInputElement;
  //Expect - field to be empty
...
test('MortgageCalculator Monthly Down Payment Term Field', () => {
  render(<MortgageCalculator />);
  const term = screen.getByTestId('mortgageMonthlyDownPaymentTerms') as HTMLInputElement;
  //Expect - field to be on first selection
  expect(term.value).toBe('0');
  fireEvent.change(term, { target: { value: '10' } });
...
test('MortgageCalculator Monthly Payment Amount', async () => {
  ...
  const calculateAmount = (principle: number, rate: number, term: number, amount?: number, months?: number) => {
    const interest = (rate + 2.5) / 100 / 12;
    const terms = term * 12;
    if (amount && months) {
      principle -= (amount * months);
    }
    return principle * interest * (Math.pow(1 + interest, terms)) / (Math.pow(1 + interest, terms) - 1);
  }

  //Mock interestRate API
  jest.spyOn(API, 'interestRate').mockImplementation(() => 
    Promise.resolve({rate: 2.75})
  );
  ...
  const principle = screen.getByTestId('mortgagePrinciple') as HTMLInputElement;
  const amount = screen.getByTestId('mortgageMonthlyDownPaymentAmount') as HTMLInputElement;
  const monthlyTerm = screen.getByTestId('mortgageMonthlyDownPaymentTerms') as HTMLInputElement;
  ...
  const payment1 = calculateAmount(1000000.00,5.00,10).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
  ...
  const payment2 = calculateAmount(1000000.00,5.00,10,100.00,12).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
  //Expect - calculated monthly payment to be present
  ...
  expect(screen.getByText('Your Data has been submitted. Our rep will reach out to you shortly.')).toBeDefined();
